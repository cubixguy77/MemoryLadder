...
public class RandomWordsGameManager extends Fragment implements GameManager {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            viewModel.getGamePhase().observe(getViewLifecycleOwner(), this::render);

            viewModel.getColumnNum().observe(getViewLifecycleOwner(), columnNum -> wordList.smoothScrollToPosition(0));
            ...
            viewModel.getVisibleMemorySheet().observe(getViewLifecycleOwner(), memorySheet -> {
                if (viewModel.getGamePhaseValue() == GamePhase.MEMORIZATION)
                    memoryAdapter.setMemorySheet(memorySheet);
            });

            viewModel.getVisibleRecallSheet().observe(getViewLifecycleOwner(), recallSheet -> {
                if (viewModel.getGamePhaseValue() == GamePhase.RECALL)
                    recallAdapter.setRecallSheet(recallSheet);
            });

            viewModel.getVisibleReviewSheet().observe(getViewLifecycleOwner(), reviewSheet -> {
